...
| 类别 | 文件 | 改动 |
|------|------|------|
| 新增 | `tests/test_storage.py` | 13 个 storage 专项测试 |
| 新增 | `storage/README.md` | storage 模块使用文档 |
| 修改 | `AGENTS.md` / `SPEC.md` / `README.md` | 同步 storage 测试覆盖 + 修复远程 README.md 残留的合并冲突标记 |
...
- **💾 历史存储**：自动生成并保存历史简报记录（JSON / SQLite 双后端，13 个专项测试覆盖）
- **📱 轻量 Web**：纯浏览器访问，无需安装
...
├── storage/      # 数据持久化（JSON / SQLite 双后端 + 13 个专项测试 + README）→ 崔锦崧
...
| [storage/README.md](storage/README.md) | storage 模块使用文档（接口表 / 调用示例 / 扩展指引） |
